...
function assertObservation(requested, observation, verbosityIgnoredWarning) {
  const failures = [];
...
async function runProbe(effort, verbosity) {
  ...
  try {
    ...
    const result = {
      requested: { model, effort, verbosity },
    ...
    assertObservation(result.requested, result.upstream, result.verbosityIgnoredWarning);
...
for (let index = 0; index < efforts.length; index += 1) {
  results.push(await runProbe(efforts[index], verbosities[index]));
}
