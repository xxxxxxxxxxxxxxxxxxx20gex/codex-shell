...
- 左侧 **Skills** 中的 **CS 内置** 目录仅收录随应用适配的技能，当前提供兔子生图 `image-gen`，用户安装后默认关闭，可按需启用或卸载。支持用户从本地目录安装自己的技能；项目、管理员和插件技能不提供独立卸载入口。
...
- 左侧 **插件** 不展示官方或第三方市场候选项，也不提供市场来源添加入口。**CS 内置** 提供可选的 `CS Office` 本地办公插件，安装后加入 PDF、Word 文档和电子表格三个 Skill；安装、状态读取和卸载均使用 Core 原生插件接口。已有其他插件继续提供详情和卸载，不删除已有来源、配置或文件。
...
- CS Office 详情页提供“安装插件”入口；未安装时仅预览 Skill，不展示启用开关。安装成功后留在详情页，重新读取已安装 Skill 的路径和状态再显示开关；安装失败保留错误并可重试。
