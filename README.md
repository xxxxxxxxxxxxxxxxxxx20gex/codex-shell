...
- 左侧 **Skills** 按 **CS 内置、个人、系统** 分组。兔子生图 `image-gen` 和 CS Docs `cs-docs` 安装前后始终留在 CS 内置组，安装后默认关闭；CS Office 安装后提供的技能也归入内置组。CS Docs 只说明 Codex Shell 当前实现，不替代官方 OpenAI/Codex 文档。个人组包含用户技能和其他插件技能；系统组汇总系统、项目和管理员技能，后两者保留来源标签。支持从本地目录安装个人技能；项目、管理员和插件技能不提供独立卸载入口。
