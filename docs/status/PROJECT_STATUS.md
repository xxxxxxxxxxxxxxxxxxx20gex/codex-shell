...
- Skills 独立安装/启停/可恢复卸载、MCP 用户配置与安全 Token 输入、CS 内置目录与已安装插件管理已接入，不展示未经适配的市场候选项。CS 内置目录提供兔子生图 `image-gen`、CS 实现说明 `cs-docs`，以及可选的 `CS Office` 办公 Skill；扩展变化刷新状态，不自动中断任务。
