...
- 当前阶段：Milestone 2 - P0 桌面编程工作台；公开稳定版本 `v0.1.7` 已启用 Tauri Updater，`main` 从 `v0.1.7` 发布提交继续开发，目前包含一次仅文档同步提交。下一版版本号待下一次版本规划确定。
...
- 下一版基线：发布冻结分支 `release/v0.1.7` 和 Tag `v0.1.7` 保持在发布提交；`main` 已从该提交继续并包含文档同步提交。后续功能直接在 `main` 开发，发布时按新的版本号重新建立发布分支和 Tag。
