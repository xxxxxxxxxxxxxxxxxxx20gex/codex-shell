...
- 当前阶段：Milestone 2 - P0 桌面编程工作台；公开稳定版本 `v0.1.7` 已启用 Tauri Updater，`main` 当前与 `v0.1.7` 发布提交一致。下一版开发从该提交继续，版本号待下一次版本规划确定。
...
- 下一版基线：工作分支为 `main`，发布冻结分支为 `release/v0.1.7`，Tag 为 `v0.1.7`；三者当前指向同一发布提交。后续功能直接在 `main` 开发并按新的版本号重新建立发布分支和 Tag。
