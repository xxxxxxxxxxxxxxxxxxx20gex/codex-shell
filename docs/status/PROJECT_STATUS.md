...
- `pnpm typecheck`：通过。
- `pnpm test`：38 个测试文件、131 项测试全部通过，包含执行过程归并与原始顺序、安全 Markdown、完整完成态残留清理、轻量模型/推理强度选择、连续回答流、命令折叠、文件变更汇总、模型切换与连接重启后同 Session 恢复、每 Turn 完全访问权限覆盖、活动/归档分叉祖先隔离、时间线滚动/导航、Diff 状态/跳转、工作区 watch 生命周期、统一反向交互、Thread 读写/退订时序、Review、MCP schema 与第二阶段 RPC 覆盖。
