...
- 时间线按 app-server 原始顺序展示用户/助手消息、推理、计划、命令、文件、MCP、动态工具、搜索、图片和子智能体活动；回答只在首个回答块首行显示亮色 2px 标记，不再显示贯穿完成回答的灰色侧轨；耗时显示为紧凑的 `s`/`m s` 格式，临时重连状态归入当前 Turn 的中间处理过程；命令默认折叠，文件修改在回答末尾汇总。
