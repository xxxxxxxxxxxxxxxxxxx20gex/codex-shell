...
- 当前阶段：Milestone 2 - P0 桌面编程工作台；公开稳定版本 `v0.1.1`
...
- 产品使用 Tauri 2、React、TypeScript 与 Rust 构建，以原版 `codex app-server` 为唯一执行核心，通过 stdio JSON-RPC 通信；`v0.1.1` 发布包暂存的是通过兼容门禁的 `codex-cli 0.153.0-alpha.5`，发布 manifest 记录实际版本、哈希和 companion binaries。生成协议类型仍以 `0.152.1` 为基线。参见 [ADR-001](../decisions/ADR-001-unmodified-codex-app-server.md) 与 [ADR-003](../decisions/ADR-003-compatible-runtime-updates.md)。
