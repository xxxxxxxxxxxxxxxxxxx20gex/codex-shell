...
- 2026-08-24：右侧 inspector 新增 Codex 风格侧边聊天；通过 `thread/fork`/`thread/start` 创建 `ephemeral` 只读线程，新增主线程与侧聊事件路由隔离、关闭退订、`Ctrl/Cmd+Alt+S` 快捷打开和最大化/恢复；加入事件路由与面板交互测试，前端回归为 57 个 Vitest 文件/251 项测试。
