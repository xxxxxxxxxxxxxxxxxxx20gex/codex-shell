...
- 最近变更：确认原版 app-server 已在隔离 CODEX_HOME 中原生维护 `logs_2.sqlite` 结构化 tracing 日志和 `sessions/YYYY/MM/DD/rollout-*.jsonl` 会话事件。当前 Tauri 仍转发 stderr 到 `app-server://log`，但前端不消费该瞬时事件；日常诊断直接读取原生日志库，不重复写一套普通文本日志。
...
- 已知问题：Runtime 二进制被 Git 忽略，公开仓库尚无可复现自动下载/构建流程；旧 CODEX_HOME 迁移仍依赖同卷 `rename`，跨卷用户目录需要单独的可恢复复制方案；installer sidecar 与崩溃恢复未验证。app-server 在 SQLite 初始化之前发生的早期 stderr 故障目前没有独立的壳层滚动文件兜底。
