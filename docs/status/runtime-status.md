...
- 最近变更：Runtime 能力未改协议；Windows Sandbox 和 CODEX_HOME 配置从通用右栏迁入设置“运行环境”，app-server Runtime Notice 与有界 stderr 日志迁入“诊断”。每次 app-server 启动仍以进程代际隔离旧 reader 线程事件，Runtime staging 仍校验固定版本 manifest 和三个 sidecar 的 SHA-256。
