...
- 最近变更：前端现在消费 app-server 的 `warning`、`configWarning`、`deprecationNotice`、Windows 世界可写警告与 Sandbox readiness/setup 生命周期；所有提示进入 50 条有界外部 Store，不重复写普通文本日志。Windows Sandbox 设置直接复用 `windowsSandbox/setupStart`。
