...
- 当前状态：公开 `v0.1.3` 安装包仍使用兼容门禁的 `codex-cli 0.153.0-alpha.5` Runtime；下一版本已接入 Tauri Updater，设置中的“检查并更新”会校验正式 GitHub Release 签名后下载并安装。发布工作流从固定 HTTPS ZIP 下载 Runtime，按 SHA-256 校验后再运行兼容门禁；ZIP 必须包含同源主 Runtime 和三个 companion binaries。签名私钥保存在 GitHub Actions Secret，Runtime 地址与 SHA-256 使用仓库 Actions Variables。该 Runtime 通过现有协议兼容门禁，但上游版本仍带有 alpha 标识。
...
- 下一步：上传经过授权的 Runtime ZIP，配置 `CODEX_SHELL_RUNTIME_URL` 与 `CODEX_SHELL_RUNTIME_SHA256`，触发 tag 工作流并在干净 Windows 用户环境验证更新后的 UAC、sandbox readiness 和 elevated 命令执行闭环。
