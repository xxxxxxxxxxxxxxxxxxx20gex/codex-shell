...
- 最近变更：Runtime 与官方桌面端对齐启用 `features.code_mode_host=true`，staging 现在把 `codex-code-mode-host.exe` 与两个 Sandbox helper 作为同版 sidecar 固定并校验 SHA-256；生成协议同步升级到该 Runtime。每次 app-server 启动仍以进程代际隔离旧 reader 线程事件。
