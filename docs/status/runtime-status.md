...
- 当前状态：公开 `v0.1.3` 安装包仍使用兼容门禁的 `codex-cli 0.153.0-alpha.5` Runtime；下一版本已接入 Tauri Updater，设置中的“检查并更新”会校验正式 GitHub Release 签名后下载并安装。个人发布使用本机 `pnpm release:package` 生成签名安装器、`.sig` 和 `latest.json` 后手动上传；Runtime 不进入 Git。该 Runtime 通过现有协议兼容门禁，但上游版本仍带有 alpha 标识。
