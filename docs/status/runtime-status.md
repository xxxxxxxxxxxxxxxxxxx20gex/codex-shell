...
- 当前状态：公开 `v0.1.3` 安装包已暂存并验证 `codex-cli 0.153.0-alpha.5` Runtime、Code Mode Host 及 elevated Windows Sandbox 的同目录 helper；manifest 记录实际版本和哈希，进程、认证和数据目录隔离已完成，NSIS 安装包可生成。设置中的“检查更新”会打开官方 GitHub 最新 Release 页面；在配置签名密钥和更新端点前，不执行静默替换。该 Runtime 通过现有协议兼容门禁，但上游版本仍带有 alpha 标识。
