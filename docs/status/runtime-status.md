...
- 当前状态：公开 `v0.1.3` 安装包仍使用兼容门禁的 `codex-cli 0.153.0-alpha.5` Runtime；下一开发版本已接入 Tauri Updater，设置中的“检查并更新”会校验 GitHub Release 签名后下载并安装。签名私钥保存在 GitHub Actions Secret，发布 runner 仍需配置经批准的 Runtime 及其同目录 helper。该 Runtime 通过现有协议兼容门禁，但上游版本仍带有 alpha 标识。
