...
- 最近变更：删除旧的 `codex-shell.workspace.v1` localStorage 读写；应用启动和“新建对话”均回到当日默认项目。空白对话可选择、切换或取消自定义项目，取消时清除旧项目的文件 mention；Session 创建后选择入口锁定，只保留自定义项目的文件浏览能力，防止隐式新建对话或修改现有 Thread `cwd`。
