...
- 最近变更：新建主会话、独立侧聊及侧聊分叉通过原生 `developerInstructions` 注入统一 CS 宿主说明，再拼接用户自定义提示词。说明区分 CS 客户端与 Codex Core、独立数据及渠道、产品与模型身份，并要求按实际工具和权限判断能力；不覆盖 Core 基础提示词或修改系统 Skill。
