...
- 最近变更：删除会因新增、归档和删除而漂移的 Session 展示编号，左侧历史与中央标题只显示稳定名称，所有操作继续使用真实 Thread ID 或 rollout 路径。历史刷新会保留当前已知的分叉祖先，避免父线程暂时不在分页结果时丢失缩进。模型启动配置触发 app-server 重启时会保存并重新打开当前 Thread，避免下一轮误建新 Session。工作区浏览器继续通过 app-server `fs/watch` 自动刷新目录和当前预览文件。
