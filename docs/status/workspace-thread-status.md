...
- 最近变更：Session 重命名、置顶、归档、恢复、删除和分叉统一迁入 `useThreadActions`，使用同步 ref 互斥锁和每次操作唯一 token；同一渲染周期内的双击、分叉期间发送/切换和其他并发 Session 变更会在发请求前被拒绝。Runtime 重置会使在途 token 失效，迟到响应不能再覆盖当前 Session 或错误清除新操作状态。归档继续直接执行，旧的归档确认类型、文案、状态分支和测试已删除，永久删除仍保留二次确认。
