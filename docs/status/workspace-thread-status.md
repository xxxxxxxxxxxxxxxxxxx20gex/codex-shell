...
- 最近变更：消息中的项目相对路径和项目内绝对路径可直接打开工作区只读预览；项目外绝对路径通过系统资源管理器定位。路径归一化明确区分工作区包含关系，拒绝 `..` 穿越，避免把模型输出中的路径当成可执行文件直接启动。Session 原子操作与永久删除确认行为保持不变；归档与永久删除共享同一套服务端移除后的队列、当前线程和历史列表清理流程，减少重复状态处理。
