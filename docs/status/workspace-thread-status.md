...
- 图片预览（2026-09-14）：项目文件中的图片可点击打开共享批注窗口，插入到主会话草稿时自动附加原图；图片窗口打开期间 Escape 不关闭底层文件浏览器。读取失败时显示错误且不提供批注入口。回填会恢复对话可见布局，不提交消息。
- 文件定位（2026-09-17）：所有专用资源管理器入口在 Rust 边界复用 Tauri Opener 原生 Shell 定位，打开文件所在目录并选中文件；目录直接打开。删除手工拼接 Explorer `/select` 参数及标准库 canonicalize，避免参数二次转义与 Windows verbatim 路径兼容问题。只接受已存在的绝对文件或目录路径。
- 定位验证（2026-09-17）：TypeScript、Cargo check、42 项 Rust 单测及严格 Clippy 通过；另显式运行默认忽略的桌面定位测试，普通路径与中文/空格路径均通过 Shell COM 确认父目录和选中文件，verbatim 路径调用通过。默认测试不弹出资源管理器。
- 文件树菜单（2026-09-16）：右键或 Shift+F10 打开目标菜单，支持预览、图片批注、添加到当前草稿、资源管理器定位和完整路径/项目相对路径/文件名复制；文件夹支持展开折叠及打开目录。二进制文件不显示预览操作，HTML 沿用文本预览。附件复用拖入路径入口，保留正文并去重，不自动发送。菜单支持方向键、Home/End、Escape、外部点击和窗口边缘避让；操作失败显示错误。
- 当前文件（2026-09-17）：文件树以选中背景和左侧 2px 动作色标记当前预览文件；从回复链接打开嵌套文件时展开父目录并滚动到可见位置，Windows 路径分隔符及大小写差异不影响高亮。项目外文件仍只显示预览，不伪造项目树节点。
- 菜单验证：2026-09-16；全套前端 332 项测试通过，TypeScript、ESLint、production build、Knip、Cargo check、42 项 Rust 单测与 Clippy 通过；菜单在 1440×900、1280×780、1024×720、900×700 验证边缘位置、键盘导航、外部点击及 reduced-motion。Windows 资源管理器的实际打开效果仍需桌面手动验收。
- 当前文件验证：2026-09-17；组件回归覆盖 `/` 与 `\\` 混用时的展开、高亮、`aria-current` 和首次滚动，四视口浏览器检查选中背景与左侧标记；完整质量检查与 Debug 构建通过，详见测试与发布状态。
