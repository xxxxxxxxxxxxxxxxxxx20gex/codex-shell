...
2. **分页历史**：打开 Session 优先使用 `thread/read(includeTurns=false)` + `thread/turns/list(itemsView=full)`，最多载入最近 200 个 turn；旧 Runtime 或不支持分页时回退到 `thread/read(includeTurns=true)`/`thread/resume`。
...
4. **Goal/Plan**：Goal 使用 `thread/goal/*` 及通知；Plan 使用原生 collaboration mode 和 `turn/plan/updated`，两者互斥逻辑由 Composer 保持。注意 `tools.update_plan.enabled` 是 TODO/checklist 工具开关，不是 Plan collaboration mode 的开关；CS 读取该字段仅用于诊断，不据此错误隐藏 Plan。运行时未提供该字段时遵循 app-server 默认值。
