...
1. **队列与引导**：CS 现有 queue/steer 交互保持不变；客户端已提供 `thread/queue/*` 原生请求封装，待协议能力在目标 Runtime 上稳定后替换本地队列调度。`turn/steer` 仍直接复用 app-server。
...
4. **Goal/Plan**：Goal 使用 `thread/goal/*` 及通知；Plan 使用原生 collaboration mode 和 `turn/plan/updated`，两者互斥逻辑由 Composer 保持。
