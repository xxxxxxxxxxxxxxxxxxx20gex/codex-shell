...
- 最近变更：时间线行为测试增加流式 Turn 高度增长继续贴底、布局驱动的滚动校正不误判为用户导航、用户主动上滑停止跟随，以及“返回最新”对齐最后一项底部的回归场景。
...
- 验证证据：2026-08-17 完整 `pnpm test:quality` 以退出码 0 完成：50 个 Vitest 文件、206 项测试、TypeScript、Vite production build、Knip、`cargo check`、12 项 Rust 单元测试、严格 Clippy 和 `git diff --check` 全部通过，生产主包约 591 KB；`pnpm desktop:build` 成功生成桌面 debug 可执行文件。
- 最后更新：2026-08-17
