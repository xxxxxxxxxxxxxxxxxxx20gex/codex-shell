...
- 最近变更：设置面板取消最大化/最小化后，替换对应单测和布局断言，覆盖默认尺寸、仅保留关闭控制、渠道编辑取消及设置分类打开/重新打开。渠道保存事务、配置原子替换与密钥失败补偿回归保留。
...
- 布局证据：2026-09-10；pnpm test:channel-layout 在 1440×900、1280×780、1024×720、900×700 通过，覆盖设置默认尺寸与视口约束、无最大化/最小化/恢复按钮、列表、独立编辑、焦点、外部点击、Escape 与 reduced-motion。真实组件配模拟回调，非 Tauri 端到端。
