...
- 最近变更：新增运行中的命令默认折叠、发送按钮 Queue/Steer 选择、排队消息直接 Steer、消息轨道点击最后一轮但未真实到底时不恢复自动跟随、Steer 分隔多个过程块时保持整轮运行状态和 app-server 原生 commentary/reasoning 连续过程流覆盖；既有普通输入排队、Session 队列隔离、文件修改末尾汇总、模型热切换、同 Session 续聊、每 Turn 权限覆盖和分叉祖先覆盖继续通过。
