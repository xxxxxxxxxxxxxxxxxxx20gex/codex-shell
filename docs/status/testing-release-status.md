...
- 最近变更：新增普通文件输入语义、UTF-8 路径范围、用户消息附件还原、Composer/历史附件预览、仅附件 optimistic Item 和旧 mention 兼容测试；固定 Runtime 和生成协议保持不变。
...
- 验证证据：2026-08-14 共 45 个 Vitest 文件、177 项测试通过；`pnpm typecheck`、Vite production build、`pnpm rust:check`、Knip 和 `git diff --check` 通过。生产主包约 571 KB，仍只有既有体积提示。Tauri debug 构建通过并生成 `src-tauri/target/debug/codex-shell.exe`；此前 Rust 单元、真实 Plan 和第三方网关 app-server smoke 证据继续有效。
- 最后更新：2026-08-14
