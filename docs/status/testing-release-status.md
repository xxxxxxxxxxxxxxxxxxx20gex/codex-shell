...
- 当前状态：公开稳定版本为 `v0.1.2`；前端 TypeScript/ESLint/Vitest、Rust check/单测/Clippy 和 Vite 生产构建均可执行；`pnpm desktop:package` 可生成带兼容门禁 Runtime 的 NSIS 安装包。设置中的“检查更新”已接入官方 Latest Release 页面，当前采用用户确认后手动下载安装，不是静默自动更新。
