...
- 最近变更：统一 `+` 菜单新增 DOM 覆盖，验证附件与六项 Codex 命令合并、命令回调、Thread 依赖禁用、外部点击/Escape 关闭和文件选择；既有 `/` 快捷命令、模型与权限测试继续通过。
...
- 验证证据：2026-08-14 共 47 个 Vitest 文件、191 项测试通过；`pnpm typecheck`、Vite production build、`pnpm rust:check` 和 Knip 通过。生产主包约 575 KB，仍只有既有体积提示。迁移后 Rust 从 clean target 全量重编译通过；此前 Rust 单元、真实 Plan 和第三方网关 app-server smoke 证据继续有效。
- 最后更新：2026-08-14
