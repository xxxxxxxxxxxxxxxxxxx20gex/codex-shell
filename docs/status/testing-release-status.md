...
- 最近变更：时间线和 Thread Hook 测试增加历史回答分叉入口、选中 Turn ID 透传，以及最新 Turn 执行中仍可从已完成历史 Turn 分叉的回归场景。
...
- 验证证据：2026-08-17 完整 `pnpm test:quality` 以退出码 0 完成：50 个 Vitest 文件、209 项测试、TypeScript、Vite production build、Knip、`cargo check`、12 项 Rust 单元测试、严格 Clippy 和 `git diff --check` 全部通过，生产主包约 591 KB；`pnpm desktop:build` 成功生成桌面 debug 可执行文件。
- 最后更新：2026-08-17
