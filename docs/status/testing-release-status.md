...
- 最近变更：新增渠道保存事务与高级设置激活回归，覆盖重启完成、失败重试、重复提交、执行请求互斥、密钥提交与配置失败、未知模型和未知服务层级；Rust 增加配置原子替换、稳定迁移身份、密钥失败补偿测试。
...
- 布局证据：2026-09-10；pnpm test:channel-layout 在 1440×900、1280×780、1024×720、900×700 通过，覆盖设置列表、独立编辑、最大化、最小化恢复、焦点、外部点击、Escape 与 reduced-motion。真实组件配模拟回调，非 Tauri 端到端。
