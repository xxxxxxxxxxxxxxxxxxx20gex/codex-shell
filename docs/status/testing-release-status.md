...
- 最近变更：补充可重试错误生命周期测试，覆盖重连提示随匹配 Turn 结束清除、不同 Session 状态隔离；同时保留 app-server 监听器初始化失败后的可重连测试和反向请求 handler disposer 隔离测试。
...
- 验证证据：2026-08-18 重连提示修复后完整 `pnpm test:quality` 以退出码 0 完成：ESLint 零 warning、53 个 Vitest 文件、229 项测试、TypeScript、Vite production build、Knip、`cargo check`、13 项 Rust 单元测试、严格 Clippy 和 `git diff --check` 全部通过；随后重新生成桌面 debug 可执行文件。
- 最后更新：2026-08-18
