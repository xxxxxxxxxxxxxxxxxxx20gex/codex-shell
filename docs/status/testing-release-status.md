...
- 验证证据：2026-08-21 本次滚动修复的定向 lint 与 23 项时间线测试通过；完整前端回归为 55 个 Vitest 文件/258 项测试，TypeScript、ESLint、Vite production build 通过；Rust check 与 14 项 Rust 单元测试通过。Knip 在 Oxc parser 分配 ArrayBuffer 时因当前 Windows/Node 内存分配失败（`RangeError: Array buffer allocation failed`），未产生无效代码报告。当前保留约 612.16 kB bundle size warning。
- 最后更新：2026-08-21
