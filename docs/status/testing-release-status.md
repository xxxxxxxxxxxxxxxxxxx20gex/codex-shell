...
- 最近变更：新增原生 Item 中间态和每日默认工作区测试，覆盖 Plan/MCP 进度、started/completed 清理、文档目录命名、托管路径识别，以及左侧隐藏默认目录；既有 Session、CODEX_HOME 和有界状态覆盖继续保留。
...
- 验证证据：2026-08-10 共 15 个 Vitest 文件、63 项测试通过；TypeScript 检查、Vite production build、Knip、9 项 Rust 单元测试、Cargo check 和 Clippy `-D warnings` 通过。真实 Plan smoke 证据继续有效。
- 最后更新：2026-08-10
