...
- 最近变更：新增连续回答流、命令折叠、文件修改末尾汇总、模型热切换及 Runtime 重启后同 Session 续聊、轻量模型菜单和字号可读性调整、每 Turn 权限覆盖、分叉祖先保留和移除动态 Session 编号的回归覆盖；桌面 debug 构建继续校验固定 Runtime sidecar。阶段性健康审查验证 Knip 无未使用项、jscpd 仅发现测试夹具重复。
