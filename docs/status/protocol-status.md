...
- 当前状态：稳定协议类型仍由 `codex-cli 0.152.1` Runtime 生成；公开 `v0.1.2` 安装包使用通过兼容门禁的 `codex-cli 0.153.0-alpha.5` Runtime，新增协议未自动接入 UI。Plan 使用最小化的局部实验字段适配，Runtime staging 允许新增协议但会阻止当前调用面被删除。
