...
- 当前状态：稳定协议类型已由 `codex-cli 0.152.1` Runtime 生成；Plan 使用最小化的局部实验字段适配。Runtime staging 允许新增协议，但会阻止当前调用面被删除。
- 最近变更：生成类型已与 `codex-cli 0.152.1` Runtime 同步；上游新增 Project、Thread Project 和严格审核通知，现有 CS 调用的 RPC、通知和反向请求保持不变。置顶从已移除的 `Thread.isPinned` 迁移到原生内置 Pinned Section，没有手改生成类型。
...
- 验证证据：`pnpm protocol:generate` 从 `codex-cli 0.152.1` Runtime 重新生成类型；兼容门禁确认现有调用面保留；TypeScript、Vitest 和生产构建通过，置顶客户端序列化测试确认使用 `thread/section/move`。
