...
- 最近变更：initialize 按生成类型完整声明 `experimentalApi: true`、`requestAttestation: false` 和 MCP form elicitation；`AppServerClient.startTurn` 仅在 Plan 时附加 `collaborationMode`。权限模式使用固定 Runtime 生成的完整 `SandboxPolicy` 联合类型，不再依赖省略字段来继承旧 Turn 状态。
