...
- 最近变更：重构时间线滚动控制：移除 Virtuoso 及其 `followOutput`、`isScrolling`、`atBottomStateChange` 多源滚动状态，改为单一原生容器和显式滚动策略。程序定位只通过一次 `scrollTop/scrollTo({ behavior: "auto" })` 完成；用户 scroll 会进入短暂 settle 锁，锁定期间任何消息更新都不能抢回视口；只有用户在底部稳定后或明确点击“返回最新”才恢复流式跟随。切换 Session 会重置滚动策略并定位新会话底部，已完成历史不会触发自动滚动。Turn 的处理中标识现在只绑定 `status === "inProgress"`，Goal continuation 启动瞬间不会把上一轮已完成回答误标成处理中。自动审批和终端交互仍沿用现有 Turn 活动区与完成后折叠逻辑。
