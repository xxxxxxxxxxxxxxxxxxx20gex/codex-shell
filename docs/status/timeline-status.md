...
- 最近变更：完成态时间线会把 app-server 在初始用户消息前产生的预采样活动（例如模型切换兼容压缩）展示到该用户消息之后，恢复“用户输入 → 中间过程 → 模型回答”的对话语义；仅重排 `clientId=null` 的初始消息前导 Activity，Steer 中途消息继续保持原生顺序。底部跟随与历史回答分叉行为保持不变。
