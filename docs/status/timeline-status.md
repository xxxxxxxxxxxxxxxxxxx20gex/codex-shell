...
- 最近变更：修复运行中拖动原生滚动条时与自动贴底竞争造成的上下弹跳：任何指针滚动操作开始时立即暂停自有和 Virtuoso 内置的自动跟随，拖动期间即使虚拟列表短暂报告到底也不恢复，释放后仅在真实底部恢复；补充 `lostpointercapture`、`mouseup`、窗口失焦兜底，避免原生滚动条漏发 pointerup 后永久停留在拖动态。自动审批和终端交互仍沿用现有 Turn 活动区与完成后折叠逻辑。
...
- 验证证据：DOM 测试覆盖流式贴底、主动上滑停止跟随、滚动条向下拖动暂停跟随、拖动中虚假到底不恢复、Virtuoso `followOutput` 关闭、原生拖动通过 mouseup/失焦恢复、完成态折叠及卸载清理；reducer 测试继续覆盖自动审查合并、延迟 started 不倒退、终端 stdin 脱敏、有界过程事件与 Turn 裁剪。
- 最后更新：2026-08-21
