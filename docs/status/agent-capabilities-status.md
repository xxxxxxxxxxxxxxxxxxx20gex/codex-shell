...
- 最近变更：运行中普通 Enter 使用 Shell 内存中的按 Session 有界 Queue，前一个 Turn 完成后再通过原生 `turn/start` 启动下一 Turn；`Ctrl+Shift+Enter`（macOS 同时支持 `Cmd+Shift+Enter`）只执行原生 `turn/steer`，不可 Steer 阶段会明确提示且不会退化为 Queue。前端消费 `ThreadStatus.activeFlags`，只有有真实 Turn ID 的普通 Turn 才允许 Steer。历史 Resume 不再重复写入模型或权限，所有下一 Turn 设置在 `turn/start` 原子生效。
