...
- 最近变更：内置高德 Skill 增加真实底图行程图模板，输出 PNG、图片型 PDF、预览及数据记录；绘图另需 Python、Pillow 和中文字体。底图缓存参数不符或缺少记录时拒绝复用，要求换新缓存路径。支持公交、地铁、步行、骑行与明确标注的示意段，路线白色底衬、自动线型图例和精简标签改善密集底图辨识；参考流程区分总览与景区详情页。
