...
- 最近变更：Skill 列表将已安装与未安装的内置项合并到固定位置，CS 内置组为兔子生图→高德，系统组 CS 文档置前；其余 Skill 按 name/path 排序。插件列表 CS Office 固定首项，其余按 name/id 排序；插件内 Skill 按 name/path 排序。安装状态、启停状态和 Core 返回顺序不参与排序，搜索仅过滤。个人 Skill 与第三方插件卸载仍移出列表，剩余项相对顺序不变。
