...
- 验证证据：固定 Runtime 本地假网关探针确认 `thread/goal/set` 会依次发出 Goal 更新、Thread 状态和 `turn/started` 通知并请求 `/v1/responses`；新增订阅与 reducer 测试确认 Goal 通知只更新匹配 Thread，Shell 不额外补发重复 Turn。
- 最后更新：2026-08-21
