...
- 最近变更：高德四种地址路线在起点或终点返回多个地理编码候选时退出码为 6，stderr 标明歧义端点、stdout 保留原始候选，不发起路线请求；确认后以坐标规划。两个驾车命令支持 v3 strategy（0–20）和最多 16 个有序途经坐标；关键词／周边搜索支持 page、offset（1–25），拒绝起始位置超过同一查询 200 条窗口的请求，一次只查询一页。
