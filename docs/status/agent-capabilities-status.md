...
- 最近变更：Goal 状态开始消费 Core 的 `thread/goal/updated` 与 `thread/goal/cleared` 权威通知，智能体 continuation 或其他原生入口的目标变化可同步到当前 Session；Goal 的 set/get/clear RPC 和 Plan 互斥逻辑保持不变，不增加 Shell 自建循环。
