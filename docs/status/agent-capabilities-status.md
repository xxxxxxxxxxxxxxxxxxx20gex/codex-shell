...
- 最近变更：参考本机官方办公插件 26.904.11930 的五类工作流，增加演示文稿与个人办公模板，重写五项中文说明及元数据；保留官方的模板优先、内容/结构/视觉分开验证原则，独立实现本地渲染与模板脚本，不复制官方私有运行库。市场源目录在 manifest 变化时先暂存校验、备份再原位替换，保持 Core 已登记路径稳定；失败尝试恢复旧源，不直接修改已安装缓存。已有安装需卸载重装并新建对话。
