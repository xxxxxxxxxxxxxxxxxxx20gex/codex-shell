...
- 最近变更：`/review` 支持未提交修改、基础分支、Commit、自定义要求以及 inline/detached 输出，并保留 app-server 返回的合成审查 Turn。运行中普通 Enter 改为按 Session 排队，前一个 Turn 完成后再通过原生 `turn/start` 启动下一 Turn，避免 `turn/steer` 改变当前任务方向而丢失预期回答；`Ctrl+Shift+Enter` 仍可显式调用原生 `turn/steer`。前端区分 `regular`、`review`、`compact` 和仅由状态恢复的 `unknown` 运行类型，并消费 `ThreadStatus.activeFlags`，只有有真实 Turn ID 的普通 Turn 才允许显式 Steer。MCP 面板补齐 OAuth、配置重载、工具/资源清单和最多 100000 字符的资源预览，外链只允许 HTTP(S)。
