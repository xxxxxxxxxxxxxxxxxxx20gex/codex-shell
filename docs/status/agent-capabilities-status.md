...
- 最近变更：插件 Skill 只在所属插件详情管理，独立 Skill 页面按 `pluginId` 排除插件技能，不按名称过滤；仍按 CS 内置、个人、系统分组。兔子生图和 CS Docs 安装前后固定在内置组；系统 OpenAI 官方文档明确不代表 CS 实现。`/skills` 汇总所有已启用技能并标注插件来源，关闭或卸载后不再可选。修复插件详情源路径与实际安装路径混用导致的开关失效。
