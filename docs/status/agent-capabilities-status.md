...
- 最近变更：`/review` 支持未提交修改、基础分支、Commit、自定义要求以及 inline/detached 输出，并保留 app-server 返回的合成审查 Turn。运行中的普通输入使用原生 `turn/steer` 追加到当前 Turn；前端现已区分 `regular`、`review`、`compact` 和仅由状态恢复的 `unknown` 运行类型，并消费 `ThreadStatus.activeFlags`，只有有真实 Turn ID 的普通 Turn 才允许 Steer。app-server 返回的 steer `userMessage` 按 Item 原始顺序显示，不创建新 Turn 或 Session。MCP 面板补齐 OAuth、配置重载、工具/资源清单和最多 100000 字符的资源预览，外链只允许 HTTP(S)。
...
- 验证证据：协议与 DOM 测试覆盖 Review、Steer 分流、Review/Compact 不可 Steer、activeFlags 状态保持、同 Turn 中途用户消息原生顺序、MCP OAuth/资源请求、资源截断、安全 URL 和 Review 面板交互；既有 Plan/Skills/MCP/Goal smoke 继续有效。
- 最后更新：2026-08-13
