...
- 已知问题：更改 Base URL 或 API Key 后需要重启连接才能让 Provider 与原生目录同步；保存时只用 `trim()` 判断 Base URL/模型 ID 是否为空，但仍持久化原始字符串，首尾空白可能导致 provider 或模型解析失败；第三方 Provider 尚无声明任意扩展参数 schema 的协议。
- 下一步：保存前规范化 Base URL 与模型 ID，并增加显式连接测试和兼容性诊断，区分鉴权、模型不存在和参数不兼容。
