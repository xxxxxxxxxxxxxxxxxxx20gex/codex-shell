...
- 最近变更：高级模型设置只保留渠道、模型 ID、推理摘要、回答冗余度和服务层级选项，移除 Provider 能力、Core 管理和参数解释卡片。目录外自定义模型 ID 原样保存、传递和显示，不再套用目录模型的 GPT 展示格式。Core 针对该自定义 ID 发出的预期 fallback metadata 警告不再反复显示；其他 app-server 警告仍保留。渠道保存串行执行，后端按预期配置检查冲突；需要重启时，提交前检查全部运行 Thread、主会话提交与侧聊提交，暂停新的执行 RPC，等待配置持久化、React 提交、Runtime 重启、目录校准和当前 Thread 参数同步后才成功。重启失败明确提示配置已保存，原样重试仍会重启。删除最后一个渠道只停止 Runtime。非激活渠道更新 Key 不重启。
