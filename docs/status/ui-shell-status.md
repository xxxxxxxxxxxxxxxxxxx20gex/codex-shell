...
- 最近变更：设置默认 1040×760 并受视口约束，提供最小化、最大化/还原、关闭；最小化隐藏遮罩并保留分类及草稿，右下角可恢复，不操作整个 Windows 窗口。渠道编辑独占内容区域；打开时聚焦面板，Tab 循环，最小化释放 Escape 监听。
...
- 验证证据：2026-09-10 设置改动四视口 Playwright 通过（1440×900、1280×780、1024×720、900×700），覆盖列表、编辑、最大化、最小化草稿恢复、焦点循环、外部点击、Escape、reduced-motion；使用真实组件和模拟回调，不是 Tauri 端到端。完整结果见 [测试与发布](testing-release-status.md)。
- 最后更新：2026-09-10
