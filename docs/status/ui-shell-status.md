...
- 最近变更：侧栏加入 48px 品牌头与 32px 新建对话按钮，主会话改为固定 48px 标题栏，时间线收敛到 820px 可读宽度，Composer 收敛到 860px 居中底栏；颜色、字号、控件高度统一由 `src/styles/tokens.css` 设计 token 驱动，历史会话操作统一使用 Lucide 图标。第二轮清理了工作区选择器的旧卡片外框、活动流的过小字号和检查器嵌套卡片视觉。Goal 和 Plan 仍为 Composer 底栏的单个可关闭互斥标签。
