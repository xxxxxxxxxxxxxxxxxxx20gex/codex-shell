...
- 最近变更：项目选择器改为新对话级交互：自定义项目卡片旁增加独立取消按钮，取消后恢复当日默认目录；已有 Session 隐藏切换和取消入口，仅允许浏览其固定项目。选择或取消项目不再隐式创建新对话，默认项目 Session 继续不占用左栏空间。
...
- 验证证据：项目选择器 DOM 测试覆盖默认状态、自定义项目卡片、取消回调和 Session 创建后锁定；完整门禁结果记录于测试状态文档。
- 最后更新：2026-08-14
