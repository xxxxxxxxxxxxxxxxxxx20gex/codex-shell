...
- 最近变更：采用负形核心 `CS` 字标作为桌面应用图标，并通过共享 `ProductMark` 同步用于标题栏与新对话空状态；可维护矢量母版位于 `assets/branding/cs-app-icon.svg`，Tauri 平台图标和壳内品牌标识均引用该母版。产品名称、应用 ID、数据目录和窗口标题保持 Codex Shell，不因交流简称 CS 发生迁移。
