...
- 交互校正：项目文件入口直接打开 WorkspaceExplorer；只有默认项目路径尚未准备好时才进入提示页，不再要求用户重复点击“打开项目文件”。左右抽屉开关移入对话标题栏两端，使用左右镜像的分栏图标；分隔线仅保留拖拽调整宽度，不再承载展开/隐藏按钮。左侧会话列表限制为纵向滚动，长标题和分支缩进不会再产生底部水平滚动条。
