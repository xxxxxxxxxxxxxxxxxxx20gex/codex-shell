...
- 交互校正：项目文件入口直接打开 WorkspaceExplorer，并以内嵌方式挂载在右侧 inspector，与侧边聊天共享面板生命周期；只有默认项目路径尚未准备好时才进入提示页，不再要求用户重复点击“打开项目文件”。项目文件标题栏操作统一为无边框 28px 图标按钮，最大化/恢复接入右侧栏宽度状态，操作区固定在标题栏右侧且不会被长路径挤出，关闭后返回功能入口。左右抽屉开关移入对话标题栏两端，使用左右镜像的分栏图标；分隔线仅保留拖拽调整宽度，不再承载展开/隐藏按钮。左侧会话列表限制为纵向滚动，长标题和分支缩进不会再产生底部水平滚动条。
