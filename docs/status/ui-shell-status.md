...
- 最近变更：Windows 原生装饰已关闭，改用 32px 无边框 `WindowTitleBar`；保留拖动、双击最大化/还原、最小化、最大化/小窗口切换和关闭，并通过 Tauri capability 只开放这些窗口动作。
- 当前接口：`App`、`WindowTitleBar`、三栏布局、`PreferencesPanel`、`StatusInspector`、Runtime Log/Notice、`ServerInteractionDialog`、Tauri Opener、工作区浏览器、`ComposerAddMenu`、`ComposerIntentControl`、slash/Skill/MCP/Review 面板、时间线、Diff 和模型设置。
