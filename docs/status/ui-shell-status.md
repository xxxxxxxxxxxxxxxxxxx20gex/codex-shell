...
- 最近变更：侧栏加入 48px 品牌头与 32px 新建对话按钮，主会话改为固定 48px 标题栏，时间线收敛到 820px 可读宽度，Composer 收敛到 860px 居中底栏；颜色、字号、控件高度统一由 `src/styles/tokens.css` 设计 token 驱动，历史会话操作统一使用 Lucide 图标。会话列表按设计稿恢复为悬浮显示“复制 + 更多操作”两个入口，更多操作点击后展开菜单；本轮将 Runtime 日志与 Diff 检查器中的低于 11px 字号和旧棕灰边框迁移到设计 token。Goal 和 Plan 仍为 Composer 底栏的单个可关闭互斥标签。
