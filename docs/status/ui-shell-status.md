...
- 最近变更：发送后的首包等待态与后续执行态统一为同一个“正在处理 + 实时耗时”结构，不再先出现回答侧线、“正在等待模型响应”和“生成中”后再跳变；活动状态使用尺寸固定的 5px 实心点，以透明度周期变化形成忽明忽暗的呼吸效果，不显示固定圆环或向外扩散，并服从系统减少动态效果设置。真实回答仍只在首个回答块首行显示 2px 标记；临时重连状态继续收在当前 Turn 的处理中间态。
