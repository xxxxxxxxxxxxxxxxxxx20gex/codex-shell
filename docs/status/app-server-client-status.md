...
- 最近变更：app-server 的 `error.willRetry=true` 现在作为当前 Thread 的临时重试状态展示；匹配的 Turn 完成、不可重试错误或连接重置后立即清除，不再把模型重连进度残留成 Session 永久错误。客户端监听器初始化失败时会回收 transport 并回到 `stopped`，订阅释放也会校验当前 handler，避免旧 disposer 误删新反向请求处理器。
