...
- 最近变更：固定 Runtime 的 Goal 行为已通过本地假网关探针复核：`thread/goal/set` 自带 Runtime continuation，会发出 `turn/started` 并启动模型请求。Composer 只复用该 RPC，不额外调用 `turn/start`；Plan 进入前按需复用 `thread/goal/get` 与 `thread/goal/clear` 保证互斥。
