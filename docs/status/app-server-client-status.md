...
- 最近变更：新增进程级切换互斥，存在待响应执行 RPC 时拒绝获取切换锁；暂停期间拒绝新 Turn、Review、压缩、队列、Goal 及 Thread 创建请求，读取与初始化不受影响。补齐兼容门禁 Runtime 新增的 `thread/settings/updated`、`thread/goal/updated|cleared`、`item/autoApprovalReview/started|completed` 和 `item/commandExecution/terminalInteraction` 通知订阅；Thread 权威状态按 ID 隔离，Composer 与设置逻辑读取 Core 的模型、推理、服务层级、权限和审批者状态，Goal 在打开 Session 时通过原生 get RPC 补齐；执行过程通知按 Thread ID 路由，主会话和 ephemeral 侧边聊天分别消费各自 reducer，避免后台或侧聊串流到主窗口；侧聊关闭时在现有连接可用的情况下复用原生 `turn/interrupt` 后再调用 `thread/unsubscribe`，Runtime 已停止时不为清理动作重新启动进程。应用启动会在持久化模型配置完成后才允许历史加载，保存网关配置触发的 Runtime 重启等待 React 状态提交，避免默认 Base URL 抢先启动。
