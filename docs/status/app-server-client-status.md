...
- 当前接口：基础连接、Thread/Turn、文件、Skills、MCP、模型、Review、压缩、Goal、Windows Sandbox、完整 Item 流、运行提示和 stderr 日志均通过固定 Runtime 的生成类型适配。统一附件入口继续复用 v2 `UserInput`：图片路径发送 `localImage`，剪贴板图片发送 inline `image`，普通文件和目录路径发送 `mention`；app-server 没有通用 file-attachment 变体，因此不新增自定义协议类型。
