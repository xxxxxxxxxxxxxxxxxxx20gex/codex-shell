    ...
    <div className="modal-backdrop" onMouseDown={onClose}>
      <section className="settings-modal" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <div className="settings-body">
          ...
          <div className="field"><span>回答冗余度</span><div className="segmented three">{(["low", "medium", "high"] as Verbosity[]).map((verbosity) => <button key={verbosity} className={draft.verbosity === verbosity ? "active" : ""} onClick={() => setDraft({ ...draft, verbosity })}>{verbosity === "low" ? "简洁" : verbosity === "medium" ? "适中" : "详细"}</button>)}</div><small>仅在模型支持时生效，保存后需要重启连接。当前 app-server 模型目录不公开该能力；不支持的模型会由 Core 忽略此项。</small></div>
